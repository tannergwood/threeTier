...
let prefix =
  "https://script.google.com/macros/s/AKfycbzIpG9jhT2GaXP-d6nW4h-SA20EuZ78xsWlATMEYlv1FWiRqmDG_XS1kVQYqKMFIkk/exec";
...
function start_me_up() {
  // runs when the body has loaded
  if (!prefix) {
    prefix = prompt("Enter your Google Apps Web App access point.");
  }
...
function get_data(table) {
  // get data from airtable for the specified table using the google apps script specfied atop this file
  ...
  script.src =
    prefix + "?table=" + table + "&limit=100" + "&time=" + Date.now(); // get the script from google apps script
  document.head.appendChild(script);
}

function show_data() {
  // the function that is executed when table data arrives
  if (data.exception) {
    show_exception();
    return;
  } // there was an exception in the google apps script, show the message and exit
  ...
  for (const field of data.structure) {
    // iterate across the fields in the data structure
    ...
    if (field.static) {
      static.push(field.name);
    } // if a field is marked as static, add it to the read-only list
  }
  ...
  for (const record of data.records) {
    // itreate over the records received
    add_row(record, tbody, static); // and make a row for each record received
  }
}

function add_row(record, body, static) {
  // adds a row to the table body specified using data from record
...
function add_cell(field, value, row, id, static) {
  ...
  if (static.includes(field.name)) {
    ...
    // check to see if the field is an array of values.  If so, treat it differntly.  For now, the only type we are handling is images
    for (const entry of value) {
      // loop through the entries in the array
      if (entry.url) {
        // if the entry has a url property, assume it is a picture
    ...
    if (value != undefined) {
      input.value = value;
    } // if a value was passed in, put it in the value of the INPUT tag
...
function change_value() {
  // is triggered when a user changes a value in a cell of the table
  const params = this.id.split("-"); // split the INPUT tag's id into its component parts
  switch (
    params[0] // Presentation layer data validation. params[0] is the table name
  ) {
    case "Employee": // validating fields in the employee table
      switch (
        params[2] // params[2] is the field name
      ) {
  ...
  if (document.getElementById("data-script")) {
    // if we have already fetched data using this script tag, remove the old one
  ...
  script.src =
    prefix + // set the source of the script so it will call the google apps script with the appropriate parameters
    "?mode=update&table=" +
    params[0] +
    "&record=" +
    params[1] +
...
function handle_message() {
  // deals with the data returned after updating (or failing to update) a column value on airtable through google apps script
  ...
  const tag = document.getElementById(
    data.table +
      "-" + // find the INPUT tag that has the data the user entered
      data.record +
      "-" +
      data.field
  );
  if (data.error) {
    // see if the message that came back from google apps script has an error message
    ...
    let delay = setInterval(function () {
      // initiate the function to clear the message after the nubmer of seconds specified atop this file
      if (data.value) {
        // Google apps script sent back a value to replace the users entered value.  This could be becuase it is reverting to a prior value or because it formatted what the user entered
        ...
        // there was no value passed back from google apps script
    ...
    // there is no error message, just update with any correction that came back
    if (data.value) {
